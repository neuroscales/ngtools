...
from ngtools.local.filter import ROIHandler
...
class TractAnnotationHandler(Handler):
    ...
    def _get_filter(self, path: str) -> bytes:
        ...
        tract_ids = self._get_annotations_inside(
            transform_segments, transform_bbox, trk_path)
        tract = tats.tract_bytes(tract_ids, offsets, segments)

        return tract
    
    def _get_tract(self, path: str) -> bytes:
        """Return the bytes for a single tract for a precomputed annotation."""
    ...
    def _parse_segmentation_path(self, path_str: str) -> bool:
        """Check if path is a segmentation path."""
    ...
    def _get_spatials(self, path: str) -> bytes:
        ...
        if trk_path in spatial_cache and spatial_cache[trk_path][0] == filter_layers:
            return spatial_cache[trk_path][1][int(parent)][f"{a}_{b}_{c}"]
        
        segments, bbox, offsets, _ = self._read_from_file(trk_path)
        if len(filter_layers) > 0:
            key = trk_path.split("://")[-1].replace("/", "$").replace(".", "$")
            for full_key in filtered_layers.keys():
                if key in full_key:
                    key = full_key
            ...
            transform_segments = segments.copy()
            transform_segments["start"] = np.hstack(
                [segments["start"], np.ones((
                    segments["start"].shape[0], 1))])@transform.T
            transform_segments["end"] = np.hstack(
                [segments["end"], np.ones((segments["end"].shape[0], 1))])@transform.T
            transform_bbox = np.hstack(
                [bbox, np.ones((bbox.shape[0], 1))])@transform.T
            transform_bbox = np.vstack([np.minimum(transform_bbox[0], 
                                                   transform_bbox[1]), 
                                        np.maximum(transform_bbox[0], 
                                                   transform_bbox[1])])

            tract_ids = self._get_annotations_inside(
                transform_segments, transform_bbox, trk_path)
            alltracts = len(tract_ids) == np.max(segments["streamline"])

            if not alltracts:
                tract_ids.sort()
                tract_counter = 0
                mask = np.full(len(segments), False)
                for i in range(len(tract_ids)):
                    mask[offsets[tract_ids[i]-1]:offsets[tract_ids[i]]] = True
                    
                segments = segments[mask].copy()
                tract_size = [offsets[i+1] - offsets[i] for i in range(len(offsets)-1)]
                counter = 0
                tract_counter = 0
                offsets = offsets.copy()
                for i in range(len(offsets)):
                    offsets[i] = counter
                    if tract_ids[tract_counter] == i:
                        counter += tract_size[i]
                        tract_counter = min(tract_counter+1, len(tract_ids)-1)


        spatial = tau.get_spatials(segments, bbox, offsets,
                                self.grid_densities)
        spatial_cache[trk_path] = [filter_layers.copy(), spatial]
    ...
    def _get_segmentation_info(self, path: str) -> dict:
        """Get the segmentation info file."""
        if info_cache.get(path) is not None:
            return info_cache[path]
    ...
    def _get_segmentation(self, path: str) -> bytes:
        """Return a blank segmentation layer."""
    ...
    def _get_annotations_inside(
        ...
        streamline_list = np.array([], dtype=int)
        if len(filter_layers) == 0 or \
            filter_layers[0].name[
                :len("::filter::subtraction")] == "::filter::subtraction":
            streamline_list = np.array(range(1, np.max(segments["streamline"]+1)), 
                                        dtype=int)
        ...
        for filter_layer in filter_layers:
            ...
            if isinstance(source, ng.viewer_state.LayerDataSources):
                ...
                transform = np.array(source.transform.to_json().get(
                    "matrix", np.eye(4)[:-1]))
            new_streamlines = np.array([], dtype=int)
            if len(filter_layer.annotations) > 0:
                annotation = filter_layer.annotations[0]
                if annotation.type == "axis_aligned_bounding_box":
                    new_streamlines = self.roi_handler.axis_aligned_bounding_box(
                        annotation, transform, bbox, dimensions, spatial, segments)

                elif annotation.type == "ellipsoid":
                    new_streamlines = self.roi_handler.ellipsoid(
                        annotation, transform, bbox, dimensions, spatial, segments)
            
            if filter_layer.name[
                :len("::filter::subtraction")] == "::filter::subtraction":
                streamline_list = streamline_list[
                    ~np.isin(streamline_list, new_streamlines)]
            else:
                streamline_list = np.unique(
                    np.concatenate([streamline_list, new_streamlines]))
